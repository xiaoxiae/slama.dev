...
pdf: true
description: Poznámky z přednášky Diskrétní a spojitá optimalizace (Martin Loebl +
  Milan Hladík, 2021/2022).
toc: true
language: cs
---
...
{{< math "definition" "sudá podmnožina hran" >}} podmnožina hran \(E' \subseteq E\) je sudá, právě když \(H = (V, E')\) má pouze sudé stupně.{{< /math >}}

{{< math "definition:" "matice incidence" >}} grafu \(G = (V, E)\) je matice \(I_G \in \mathbb{F}_2^{|V| \times |E|}\) t.ž. \[\left(I_G\right)_{v,e}=\begin{cases} 1 & v \in e \\ 0 & \text{jinak} \end{cases}\]{{< /math >}}

![](incidence-matrix.svg)
...
\[\begin{aligned}
	...
	&< |A'| + |A \setminus A'| \\
	&= |A| \implies A \not\in \mathcal{S} & \Rightarrow\!\Leftarrow
\end{aligned}\]
...
- \((\mathrm{R2'})\): přímo vyplývá z R2 (\(r(A) \le r(A \cup \left\{x\right\})\))
{{< /math >}}
...
{{< math "proof" "\(\Rightarrow\)" >}}platí přímo z definice submodularity (prostě tam dosadíme){{< /math >}}
...
{{< math "definition" "úloha kombinatorické optimalizace" >}}je dán množinový systém \((X, \mathcal{S})\) a váhová funkce \(w : X \mapsto \mathbb{Q}\). **Úloha kombinatorické optimalizace** je najít \(A \in \mathcal{S}\) t.ž. \[w(A) = \sum_{v \in A} w(v) = w^T \chi_A\] je **maximální** (\(\chi_A\) je charakteristický vektor \(A\), který je \(1\) pro \(v \in A\) a \(0\) jindy).{{< /math >}}
...
{{< math "observation" >}}algoritmus je polynomiální (pro rozumné předpoklady na reprezentaci matroidu).{{< /math >}}

{{< math "observation" >}}pro výše uvedené příklady nemusí algoritmus vrátit optimální řešení.{{< /math >}}

{{< math "theorem" "hladový algoritmus na matroidech" >}}nechť \((X, \mathcal{S})\) je dědičný množinový systém a \(\emptyset \in \mathcal{S}\). Pak hladový algoritmus vyřeší správně úlohu kombinatorické optimalizace pro **každou funkci** \(w \iff (X, \mathcal{S})\) je matroid.{{< /math >}}
...
{{< math "remark" >}}tvrzení by platilo triviálně, pokud by se jednalo o prostou funkci (tedy pouze přejmenování prvků matroidu). Zajímavé je to, že některé prvky se mohou zobrazit na jiné a nezávislé množiny se tak zmenší, ale matroidnost se zachová.
{{< /math >}}
...
{{< math "proof" >}}\(A\) můžeme rozšířit na bázi \(B^*\) duálu, přičemž bude stále platit \(r(X - B^*) = r(X)\), protože \(X - B^*\) je báze \(\mathcal{M}\). Tohle můžeme dělat i když jsme další větu ještě nedokázali, jelikož pro \(\mathcal{M}^*\) platí axiomy 1 a 2 matroidu, jelikož jsme ho definovali jako báze.{{< /math >}}
...
- **conezávislá**, pokud je nezávislá v \(M^*\){{< /math >}}

{{% float_box %}}
V grafu jsou to kružnice.
{{% /float_box %}}

{{< math "definition" "kružnice" >}}nechť \(\mathcal{M}\) je matroid. Pak \(Y \subseteq X\) je kružnice, je-li minimální (\(\subseteq\)) **závislá** množina.{{< /math >}}

{{< math "theorem" >}}\(Y \subseteq X\) je cokružnice (kružnice v duálu) \(\iff Y\) je min (\(\subseteq\)) protínající **každou bázi**.{{< /math >}}
...
{{< math "consequence" >}}nechť \(G\) graf souvislý. Pak cokružnice \(\mathcal{M}_G\) jsou přesně **minimální hranové řezy**. {{< /math >}}

#### Perfektní párování
{{< math "remark" >}}o tomto tématu jsem vytvořil [YouTube video](https://www.youtube.com/watch?v=3roPs1Bvg1Q), které algoritmus shrnuje.{{< /math >}}

{{< math "definition" "párování" >}}nechť \(G = (V, E)\) graf. Pak \(M \subseteq E\) je párování \(\iff \forall e \neq e' \in M\) platí \(e \cap e' = \emptyset\).{{< /math >}}

{{< math "definition:" "největší párování" >}} pokud \(|M|\) je maximální.{{< /math >}}

{{< math "observation" >}}je rozdíl mezi **maximálním** párováním (počítá se do **inkluze**) a **největším** (počítá se do **velikosti**), jelikož si hrany můžeme hladově vybrat špatně (viz lichá cesta).{{< /math >}}

{{< math "definition" "perfektní párování" >}}pokud \(|M| = |V| / 2\){{< /math >}}

{{< math "definition" "pokrytí" >}}vrchol je \(M\)-pokrytý, pokud je v nějaké hraně z párování.{{< /math >}}

{{< math "definition:" "defekt" >}} \(\mathrm{def}(M)\) je počet \(M\)-nepokrytých vrcholů{{< /math >}}
...
{{< math "theorem" "Tutte-Berge" >}}Pro výše uvedené pozorování platí rovnost.{{< /math >}}

{{< math "proof" >}} stačí najít párování \(M\) t.ž. platí rovnost.

Najdeme algoritmem (Edmonds), který najde maximální párování

- postupně zvětšujeme párování \(M\):

1. \(M\) perfektní \(\implies\) je maximální a věta platí (\(A = \emptyset\))
2. \(M\) má \(M\)-nepokrytý vrchol \(r\): budujeme alternující strom \(T\) tím, že střídavě přidáváme hrany:
...
{{< math "consequence" "Tutte" >}}\(G\) má perfektní párování právě tehdy, když \[\forall A \subseteq V: \mathrm{lc}(G - A) \le |A|\]{{< /math >}}
...
{{< math "example" >}}\(G = (V_1, V_2, E)\) bipartitní graf, \(A^{|V_1| \times |V_2|}\) \(0/1\) matice podle toho, jaké obsahuje hrany. Pak \(\mathrm{Per}(A)\) je počet perfektních párování \(G\).{{< /math >}}
...
{{< math "reminder" >}}pro další plochy je to trochu komplikovanější, ale jde to dělat podobně.{{< /math >}}
...
{{< math "observation" >}}\(|T|\) je sudé (součet stupňů je sudý){{< /math >}}
...
{{< math "proof" >}}
{{< math "observation" >}}nechť \(G'\) vznikne z \(G\) dodáním násobných hran, násobnost je podle počtu procházení minimální trasou \(P\). Potom \(G'\) má všechny stupně sudé.{{< /math >}}
