...
- the function \(f\) has a **total differential** at the point \(\mathbf{a}\), if there exists a continuous function \(\mu\) in the neighbourhood \(U\) of the point \(\mathbf{o} \in \mathbb{R}^n\), such that \(\mu(\mathbf{o}) = 0\), and numbers \(A_1, \ldots, A_n\) for which the following holds: \[f(\mathbf{a} + \mathbf{h}) - f(\mathbf{a}) = \sum_{k = 1}^{n} A_k h_k + || \mathbf{h} || \mu(\mathbf{h})\]
...
![By clicking anywhere, scrolling, or closing this notification, you agree to be legally bound by the witch Sycorax within a cloven pine.](gdpr.webp)
{.maximized-image}
