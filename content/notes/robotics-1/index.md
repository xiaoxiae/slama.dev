...
1. Introduction [[slides](slides/01_introduction.pdf)] [[tutorial -- basics](exercises/01.pdf)]
2. Robotics Components [[slides](slides/02_intro_to_robotic_components.pdf)] [[tutorial -- rotations, Grübler](exercises/02.pdf)]
3. Kinematics [[slides](slides/03_kinematics.pdf)] [[tutorial -- Grübler, DH](exercises/03.pdf)]
4. Homogeneous Transformation and DH [[slides](slides/04_homogeneous_transformation_and_dh.pdf)] [[tutorial -- DH](exercises/04.pdf)]
5. Inverse Kinematics [[slides](slides/05_inverse_kinematics.pdf)] [[tutorial -- DH, Inverse Kinematics](exercises/05.pdf)]
6. Inverse Kinematics (numerical methods) [[slides](slides/06_inverse_kinematics_numerical_methods.pdf)] [[tutorial -- Geometric Jacobian](exercises/06.pdf)]
7. Differential Kinematics [[slides](slides/07_differential_kinematics.pdf)]
8. Inverse Differential Kinematics [[slides](slides/08_inverse_differential_kinematics.pdf)] [[tutorial -- trajectory planning](exercises/08.pdf)]
9. Trajectory Planning in Joint Space [[slides](slides/09_trajectory_planning_in_joint_space.pdf)] [[tutorial -- inverse kinematics](exercises/09.pdf)]
...
### Robotics Components
{{< math "definition" "link/member" >}}individual bodies making up a mechanism{{< /math >}}

{{< math "definition" "joint" >}}connection between multiple links{{< /math >}}
...
![Open and closed kinematic chains](kinematic-chains.svg)

#### Degrees of Freedom
_This is pretty important and will most likely be on the final, so it is best to practice this. Here are [some of the examples from the exercises](grubler-examples.pdf) with my solutions (sorry for my terrible drawing)._
...
{{< math "definition" "coincident joints" >}}when there are more than two kinematic pairs in the same joint{{< /math >}}

![Coincident joints.](coincident-joints.svg)

#### Actuators
{{< math "definition" "actuator" >}}a mechanical device for moving or controlling something{{< /math >}}
...
{{< math "definition" "forward/direct kinematics" >}}the process of finding the position/orientation of the **end-effector** \((r_1, \ldots, r_m)\) given a set of joint parameters \((q_1, \ldots, q_n)\).{{< /math >}}
...
_This is a really important concept and will 99% be on the final, so it is best to practice this. Here is a [useful PDF with examples](dh-examples.pdf#page=13) (starting at page 13) where you can practice creating the DH tables and Forward Kinematics matrices._
...
![Denavit & Hartenberg Notation](dh.svg)
...
#### Inverse Kinematics
{{< math "definition" "inverse kinematics" >}}the process of finding a set of joint parameters \((q_1, \ldots, q_n)\) given the position/orientation of the **end-effector** \((r_1, \ldots, r_m)\).{{< /math >}}
...
![Workspaces example.](workspaces.png)
{.no-invert}
...
![Law of cosines illustration.](loc.svg)
{.rightFloatBox}
...
![Planar manipulator illustration.](planar-manipulator.svg)
{.rightFloatBox}

{{< math "example" >}}planar 2D arm:{{< /math >}}
...
![Singularity example.](singularities.png)
{.no-invert}
...
![Kinetostatic Duality illustration.](duality.png)
{.no-invert}
...
{{< math "definition" "path" >}}set of points in the joint space which the manipulator has to follow in the execution of the assigned motion (purely geometric).{{< /math >}}

{{< math "definition" "trajectory" >}}a path on which a timing is specified (velocities, accelerations, etc.).{{< /math >}}
...
![Trajectory Planning example.](tp.png)
{.no-invert}
...
![Cubic spline illustration](spline.png)
{.no-invert}
...
![Bang-coast-bang analysis illustration.](trapezoid.png)
{.no-invert}
...
![Velocity and force manipulability.](manipulability.png)
{.no-invert}
