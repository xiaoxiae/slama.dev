...
#### Training
- rule of thumb: **more data is better**A (nowadays \(3 \times 10^9\) images)
...
- use augmentation to avoid annual labeling
...
![U-net network structure.](/assets/introduction-to-machine-learning/linear-classifier.svg)
...
- predict \(Y\) deterministically (\(Y = f_{\theta}(x)\)) or stochastically (\(y ~ p_\theta (Y \mid X)\))
- **functional representation lemma** (aka "reparametrization trick" / "noise outsourcing")
	- every conditional probability \(p(Y \mid X)\) can be expressed by a deterministic function & independent noise: \[y ~ p(Y \mid X = x) \equiv y = f_\theta(x, \varepsilon)\] with \(\varepsilon\) drawn from a distribution (usually Gaussian) independent from \(X\); eg. \[y ~ N(Y, \mu(x), \Sigma(x)) \equiv y = \mu(x) + \Sigma(x)^{1/2} \cdot \varepsilon \qquad \varepsilon ~ N(0, \mathcal{I})\]
...
\[Y_i = X_i \beta^* + b^* + \varepsilon_i \qquad \varepsilon_i ~ N(0, \sigma^2)\] (i.e. variance is fixed but unknown)
...
- not a great way to calculate this -- expensive and also \(X^T X\) might be to degenerate
