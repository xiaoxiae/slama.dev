...
##### How good can it be?
{{< math "definition" "Bayes classifier" >}}uses Bayes rule (LHS or RHS) with true probabilities \(p^*\){{< /math >}}
- we don't know \(p^*\), we want to get as close as possible

{{< math "theorem" >}}no learned classifier using \(\hat{p}\) can be better than the Bayes classifier.{{< /math >}}
...
![Derivation of an ellipse from a unit circle.](ellipses.svg)
...
{{< math "definition" "filter" >}}consumes an image and outputs another "filtered" image{{< /math >}}
...
{{< math "theorem" >}}any such filter can be written as a convolution integral, i.e. \[\tilde f(t) = \int_{-\infty}^{\infty} f(t') \cdot \underbrace{g(t - t')}_{\text{filter kernel}} dt' = (f \otimes g) (t)\]{{< /math >}}
...
![Matched filter example.](matched-filter.webp)
...
![LeNet network diagram.](lenet.png)
...
![U-net network structure.](u-net.webp)
...
_The lecture here has an interlude into computer tomography, which can be solved using least-squares. Most of it is explained in [homework 4](hw4.pdf) so I'm not going to repeat it here._
...
![Bias-variance tradeoffs.](bias-variance-tradeoffs.webp)
