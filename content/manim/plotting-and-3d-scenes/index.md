...
We'll use the builtin classes from the {{< doc "manim" "boolean_ops" "reference/manim.mobject.geometry.boolean_ops.html" >}} file, namely {{< doc "manim" "Difference" "reference/manim.mobject.geometry.boolean_ops.Difference.html" >}}, {{< doc "manim" "Intersection" "reference/manim.mobject.geometry.boolean_ops.Intersection.html" >}}, {{< doc "manim" "Union" "reference/manim.mobject.geometry.boolean_ops.Union.html" >}} and {{< doc "manim" "Exclusion" "reference/manim.mobject.geometry.boolean_ops.Exclusion.html" >}}.

```python {file="04-boolean-operations-example.py"}
```

{{< video "manim" "04-boolean-operations-example" >}}

When using the aforementioned classes, it is important to keep in mind that they are restricted to vector objects (the {{< doc "manim" "VMobject" "reference/manim.mobject.types.vectorized_mobject.VMobject.html" >}} class) with **non-zero area**, meaning that intersecting two intersecting lines **does not** produce a point (although it geometrically should).
...
We'll be mainly using the {{< doc "manim" "Axes" "reference/manim.mobject.graphing.coordinate_systems.Axes.html" >}} class and its parent {{< doc "manim" "CoordinateSystem" "reference/manim.mobject.graphing.coordinate_systems.CoordinateSystem.html" >}}.
...
{{< video "manim" "04-discontinuous-graph-example" >}}

#### Parametric graphs

The other, more general way to plot a graph is parametrically via {{< doc "manim" "plot_parametric_curve" "reference/manim.mobject.graphing.coordinate_systems.CoordinateSystem.html#manim.mobject.graphing.coordinate_systems.CoordinateSystem.plot_parametric_curve" >}} -- we're also defining the function using an expression, but it is a single parameter function returning the corresponding pair of coordinates.

```python {file="04-parametric-graph-example.py"}
```

{{< video "manim" "04-parametric-graph-example" >}}

#### Line graphs

Besides defining the graph in terms of expressions, it is also possible to define it using the raw values themselves via {{< doc "manim" "plot_line_graph" "reference/manim.mobject.graphing.coordinate_systems.Axes.html#manim.mobject.graphing.coordinate_systems.Axes.plot_line_graph" >}}.

```python {file="04-line-graph-example.py"}
```

{{< video "manim" "04-line-graph-example" >}}
...
To render the scene in 3D, we'll have to use {{< doc "manim" "ThreeDScene" "reference/manim.scene.three_d_scene.ThreeDScene.html" >}}.

```python {file="04-axes3d-example.py"}
```
...
Besides the {{< doc "manim" "ThreeDAxes" "reference/manim.mobject.graphing.coordinate_systems.ThreeDAxes.html" >}} object used to work with the 3D axes, Manim also contains a number of 3D {{< doc "manim" "primitives" "reference/manim.mobject.three_d.three_dimensions.html" >}} that you can use to create more complex 3D scenes.

```python {file="04-rotation-3d-example.py"}
```

{{< video "manim" "04-rotation-3d-example" >}}
...
{{< video "manim" "04-basic-3d-example" >}}
...
The main one is the {{< doc "manim" "CubicBezier" "reference/manim.mobject.geometry.arc.CubicBezier.html" >}}, which can be used to model the path and animated by {{< doc "manim" "MoveAlongPath" "reference/manim.animation.movement.MoveAlongPath.html" >}}.
...
```python {file="04-bezier-example.py"}
```

{{< video "manim" "04-bezier-example" >}}
...
{{< details "Author's Solution" "04-binomial-distribution-simulation.py" >}}{{< /details >}}
