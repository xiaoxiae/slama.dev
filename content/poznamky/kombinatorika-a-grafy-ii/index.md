...
{{< math "definition" >}}Párování v \(G = \left(V, E\right)\) je \(M \subseteq E\) t. ž. \(\forall v \in V \exists \le 1\) hrana \(e \in M: v \in e\){{< /math >}}

- **maximální** (do inkluze) -- přidání další hrany pro dané párování už není možné; v přednášce nás nezajímá
- **největší** -- \(\mathrm{max}(|M|)\)

{{< math "definition:" "volný vrchol" >}} (vzhledem k \(M\)) je vrchol, kterého se nedotýká žádná hrana párování.{{< /math >}}

{{< math "definition:" "střídavá cesta" >}} (vzhledem k \(M\)) je cesta, na které se střídají hrany v párování a hrany mimo párování: \(u_0, \ldots, u_k\), kde každá sudá/lichá hrana je v \(M\), lichá/sudá není v \(M\){{< /math >}}

- **volná** střídavá cesta (VSC) -- krajní vrcholy jsou volné (vůči párování)
- \(\Rightarrow\) obsahuje lichý počet hran, sudý počet vrcholů

{{< math "claim" >}}Nechť \(G = \left(V, E\right)\) je graf, \(M\) párování v \(G\). Pak \(G\) obsahuje VSC (vzhledem k \(M\)), právě když \(M\) není největší párování v \(G\).{{< /math >}}
...
\centering
\fbox{\includesvg{..alter.svg}}
\end{wrapfigure}
-->
- \(\Rightarrow\) pokud \(M\) má VSC, mohu \(M\) zvětšit prohozením hran

- \(\Leftarrow\) pro spor nechť \(M'\) je párování v \(G\) t. ž. \(|M'| > |M|\)
...
{{< math "definition" "stonek" >}} střídavá cesta z \(v_1\) (i nulové) délky končící volným vrcholem (dál od květu){{< /math >}}
- \(v_1\) může (a nemusí) být volný vrchol -- stačí, aby byl volný vzhledem ke květu

{{< math "definition" "kytka" >}} květ + stonek{{< /math >}}

![](kytka.svg)

{{< math "definition" "kontrakce hrany" >}} Nechť \(G = \left(V, E\right)\) je neorientovaný graf a \(e = \left\{u, v\right\}\) jeho hrana. Zápis \(G . e\) označuje graf vzniklý z \(G\) kontrakcí („smrštěním“) hrany \(e\) do jednoho vrcholu:{{< /math >}}

![](kontrakce.svg)

{{< math "claim" >}}Nechť \(C\) je květ v grafu \(G\). Potom párování \(M\) v \(G\) je maximální, právě když \(M \setminus E(C)\) je maximální párování v grafu \(G . C\), tj. s květem \(C\) zkontrahovaným do jediného vrcholu. Navíc pokud znám VSC pro \(M . C\), tak v poly. čase najdu VSC pro \(M\) v \(G\).{{< /math >}}
...
{{< math "proof" >}}nejvýše \(n\)-krát se vždy zrekurzíme s tím, že při každém rekurzení prohledáme celý graf (\(\mathcal{O}(n + m)\)). Tohle celé opakujeme nejvýše dokud nejsou všechny vrcholy zpárované, tedy \(n\)-krát.  {{< /math >}}

### 2. přednáška

{{< math "definition" "perfektní párování" >}}Párování \(M\) je perfektní, pokud neexistuje v \(G\) žádný volný vrchol.{{< /math >}}

#### Tutteova věta
{{< math "definition" "Tutteova podmínka" >}} \(\forall S \subseteq V: \mathrm{odd}(G - S) \le |S|\)
- \(\mathrm{odd}\) je počet lichých komponent grafu{{< /math >}}.

{{< math "theorem" "Tutteova věta" >}} \(G\) má perfektní párování \(\iff\) platí Tutteova podmínka.{{< /math >}}
...
- **indukční předpoklad:** \(G\) má nehranu a každý graf na \(V\) s počtem hran alespoň o 1 větší než \(|E|\) a platí TP, pak má perfektní párování
...
- {{< math "observation" >}}přidáním hrany do grafu se neporuší TP (\(\forall S \subseteq V\) počet lichých komponent \(G - S\) buď klesne o \(2\) nebo zůstane stejný).{{< /math >}}
...
{{< math "theorem" "Petersen" >}} každý \(3\)-regulární \(2\)-souvislý (vrcholově i hranově, pro 3-regulární grafy je to to samé; alternativně můžeme říct graf bez mostů a artikulací) graf má PP.{{< /math >}}
...
{{< math "lemma" "o kontrahovatelné hraně = LoKH" >}} Nechť \(G\) je vrcholově \(3\)-souvislý různý od \(K_4\). Potom \(G\) obsahuje hranu t. ž. \(G \setminus e\) je 3-souvislý.{{< /math >}}
...
{{< math "lemma" "pomocné" >}} \(\forall e = \left\{x, y\right\} \exists z_e \in V \setminus \left\{x, y\right\}\) t. ž. \(\left\{x, y, z_e\right\}\) tvoří vrcholový řez v G, navíc každý z \(\left\{x, y, z_e\right\}\) má alespoň jednoho souseda v každé komponentě \(G \setminus \left\{x, y, z_e\right\}\).{{< /math >}}
...
- {{< math "observation" "které platí" >}}\(S\) minimální vrcholový řez \(G\), pak každý vrchol \(S\) má souseda v každé komponentě \(G \setminus S\) -- když to pro nějaký \(v\) neplatí, tak \(S \setminus v\) je pořád řez{{< /math >}}
...
{{< math "theorem" "Tutteova charakterizace 3-souvislých grafů" >}} Graf \(G\) je 3-souvislý \(\iff\) existuje posloupnost \(K_4 \cong G_0,  G_1, \ldots, G_n \cong G\) t. ž. \(\forall i \in [n], G_{i - 1}\) vznikne z \(G_i\) kontrakcí hrany, navíc \(G_i\) má všechny vrcholy stupně \(\ge 3\).{{< /math >}}
...
Obměnou nechť \(G_i\) má vrcholový řez velikosti 2, označme ho \(R = \left\{x,y\right\}\). Pak každá komponenta \(G_i - R\) má alespoň 2 vrcholy (osamocený vrchol \(z\) mohl sousedit jen s řezem, ale ten je velikosti 2, což je spor se stupněm vrcholů \(\ge 3\) pro \(z\)).
...
{{< math "definition" "minor" >}} Nechť \(H, G\) jsou grafy. Pak \(H\) je minor \(G\) (nebo že \(G \) obsahuje \(H\) jako minor), značíme \(H \preceq G\), pokud \(H\) lze získat z \(G\) posloupností mazání vrcholů, mazání hran nebo kontrakcí hran.{{< /math >}}

- {{< math "observation" >}}\(\preceq\) je tranzitivní (prostě spojím posloupnosti operací){{< /math >}}
- {{< math "observation" >}}\(H\) podgraf \(G \Rightarrow H\) minor \(G\){{< /math >}}
	- podgraf vzniká přesně mazáním vrcholů a mazáním hran
- {{< math "observation" "spíš fakt" >}}\(G\) rovinný \(\Rightarrow\) jeho minory jsou také rovinné{{< /math >}}
...
- \(k_v(G) \ge 3\Rightarrow\) použijeme lemma o kontrahovatelné hraně: \(\exists e = \left\{x, y\right\}\) t. ž. \(G \setminus e = G'\) je \(3\)-souvislý
	- {{< math "observation" >}}\(G'\) nemůže obsahovat \(K_5\) ani \(K_{3, 3}\) jako minor (kontrakcí něčeho, co je nemělo, je nevytvoříme){{< /math >}}
	- \(\mathcal{G}' \ldots\) rovinné nakreslení \(G'\) (existuje z IP)
	- \(G'' = G' - v_e\) (vrchol vzniklý kontrakcí \(e\)) \( = G - \left\{x, y\right\}\)
		- {{< math "observation" >}}\(G''\) bude \(2\)-souvislý (protože \(G'\) je \(3\)-souvislý a \(G''\) vznikne odebráním vrcholu){{< /math >}}
		- {{< math "observation" >}}taky rovinný (odebráním mi žádný minor nevznikne){{< /math >}}
...
- zbytek -- nenastane ani (1), ani (2)
	...
	- \(y\) nakreslím do té správné stěny, spojím s \(b_i\) a mám hotovo

![](p3.svg)
![](p4.svg)

#### Kreslení grafů na plochy
{{< math "definition" >}}Nechť \(X \subseteq \mathbb{R}^n, Y \subseteq \mathbb{R}^m\). Potom homeomorfismus z \(X\) na \(Y\) je funkce \(f: X \mapsto Y\), která je spojitá, bijekce a \(f^{-1}\) je spojitá. \(X, Y\) jsou homeomorfní (\(X \cong Y\)) pokud mezi nimi existuje homeomorfismus.{{< /math >}}
- něco jako isomorfismus u grafů (\(X \cong Y\) znamená, že se chovají stejně)

{{< math "definition" "plocha" >}} kompaktní (uzavřená, omezená), souvislá (např. oblouková -- každé dva body můžu propojit obloukem), \(2\)-rozměrná varieta bez hranice (dostatečně malé okolí každého bodu je homeomorfní otevřenému okolí v \(\mathbb{R}^2\)).{{< /math >}}
- např. sféra v \(\mathbb{R}^3\) nebo torus v \(\mathbb{R}^3\)
- není to např.
	- \(\mathbb{R}^2\), jelikož není kompaktní (omezená)
	- čtverec s hranicí, jelikož pro každý krajní bod není homeomorfní \(\mathbb{R}^2\)
...
Pro \(g \in \left\{0, 1, \ldots\right\}\) nechť \(\sum_g\) značí plochu vzniklou ze sféry přidáním \(g\) uší, tak říkáme, že \(\sum_g\) je **orientovatelná plocha** rodu \(g\).

Pro \(g \in \left\{1, 2, \ldots\right\}\) nechť \(\prod_g\) značí plochu vzniklou ze sféry přidáním \(g\) křížítek, tak říkáme, že \(\prod_g\) je **neorientovatelná plocha** rodu \(g\).

{{< math "fact" >}}Každá plocha je homeomorfní právě jedné ploše z posloupnosti \(\sum_0, \prod_1, \sum_1, \prod_2,\ldots\){{< /math >}}
- máme tu skryté tvrzení, že žádné dvě z této posloupnosti nejsou homeomorfní.

{{< math "fact" >}}Přidám-li ke sféře (\(= \Sigma_0\)) \(k \ge 0\) uší a \(l \ge 1\) křížítek, vznikne **neorientovatelná plocha** homeomorfní \(\prod_{2k + l}\) (\(\approx\) „přidání dvou křížítek je jako přidání ucha,“ **pokud** už tam bylo \(\ge 1\) křížítko){{< /math >}}
...
{{< math "definition" "stěna nakreslení" >}} souvislá komponenta \(\Gamma \setminus \left(\left(\bigcup_{e \in E}\varphi(e)\right) \cup \left(\bigcup_{x \in V}\varphi(x)\right)\right)\){{< /math >}}
- prostě souvislé komponenty toho, když odeberu všechna nakreslení hran a vrcholů

{{< math "definition" "buňkové nakreslení" >}} každá stěna je homeomorfní otevřenému kruhu v \(\mathbb{R}^2\).{{< /math >}}

![](torus.svg)

{{< math "reminder" >}}\(G = (V, E)\) souvislý \(\Rightarrow\) v každém rovinném nakreslení platí \(|V| - |E| + S = 2\) {{< /math >}}
...
{{< math "theorem" "zobecněná Eulerova formule" >}}Nechť máme nakreslení grafu \(G = (V, E)\) na ploše \(\Gamma\), které má \(S\) stěn. Pak \(|V| - |E| + |S| \ge \Chi(\Gamma)\). Pokud je buňkové, tak dokonce \(|V| - |E| + |S| = \Chi(\Gamma)\).{{< /math >}}
...
- {{< math "observation" >}}\(k \ge 1\), jinak by stěna obsahující \(K\) nebyla buňka{{< /math >}}
...
Vytvoříme \(G''\) přidáním cest délky \(2\) k sousedním vrcholům z předchozího kroku. Vznikne tím kružnice \(C\) obcházející \(K\).
- \(v(G'') = v(G') + 2k\)
- \(e(G'') = e(G') + 4k\)
...
{{< math "claim" >}}Nechť \(\Gamma\) je plocha, \(\Gamma \neq \Sigma_0\), nechť \(G\) je graf nakreslený na \(\Gamma\), potom \(G\) obsahuje vrchol stupně \(\le \left\lfloor \frac{5 + \sqrt{49 - 24\Chi(\Gamma)}}{2} \right\rfloor\){{< /math >}}
...
{{< math "consequence" "Heawoodova formule, 1890" >}} Pokud \(\Gamma \not\cong \sum_0\), tak každý graf nakreslitelný na \(\Gamma\) je nejvýš \(H(\Gamma) = 1 + \left\lfloor \frac{5 + \sqrt{49 - 24 \Chi(\Gamma)}}{2} \right\rfloor = \left\lfloor \frac{7 + \sqrt{49 - 24 \Chi(\Gamma)}}{2} \right\rfloor\)-obarvitelný{{< /math >}}
...
{{< math "definition" >}}\(G\) je \(d\)-degenerovaný \(\equiv\) každý podgraf \(H\) grafu \(G\) má \(\delta(H) \le d\){{< /math >}}
...
{{< math "observation" >}}\(G\) je \(\Delta(G)\)-degenerovaný (triviálně) \(\Rightarrow \Chi(G) \le \Delta(G) + 1\) (z pozorování výše){{< /math >}}

- s rovností platí např. pro úplné grafy a liché cykly

{{< math "lemma" >}}\(G\) souvislý graf a \(\delta(G) < \Delta(G)\), pak \(\Chi(G) \le \Delta(G)\){{< /math >}}
...
{{< math "theorem" "Slabá věta o perfektních grafech, 1972" >}}\(G\) je perfektní \(\iff\) \(\bar{G}\) je perfektní.{{< /math >}}
...
{{< math "definition" "chordální graf" >}}Graf je chordální, pokud neobsahuje \(C_k, k \ge 4\) jako in. podgraf.{{< /math >}}
- alternativní pohled vycházející ze jména: každá kružnice má _chordu_ (tětivu)

{{< math "definition" >}}Nechť \(x, y\) dva nesousední vrcholy \(G\). \(R\) je \(x{\text -}y\)-řez, pokud je to řez takový, že \(x, y\) patří do různých komponent \(G \setminus R\).{{< /math >}}

{{< math "claim" >}}\(G\) je chordální \(\iff\) pro každé dva nesousední vrcholy \(x, y \in V, x \neq y\) existuje \(x{\text -}y\)-řez, který je klika.{{< /math >}}

{{< math "proof" >}} \(\Leftarrow\) nechť \(G\) není chordální, tedy obsahuje indukovanou kružnici délky alespoň \(4\). Uvážíme-li dva její nesousední vrcholy, tak jakýkoliv řez musí obsahovat vrcholy z horní a dolní cesty mezi \(x\) a \(y\). Ty nesousedí, tedy řez nebude klika.
...
{{< math "definition" >}}Vrchol \(x\) je v grafu \(G\) simpliciální, pokud jeho sousedství \(N_G(x)\) tvoří kliku \(G\).{{< /math >}}

{{< math "theorem" >}}Každý chordální graf (kromě prázdného) obsahuje simpliciální vrchol.{{< /math >}}
- dokážeme pomocí silnějšího tvrzení

{{< math "theorem" >}}Každý chordální graf je buď úplný, nebo obsahuje dva nesousední simpliciální vrcholy.{{< /math >}}
...
\begin{wrapfigure}{R}{0.2\textwidth}
\centering
\fbox{\includesvg{..dog.svg}}
\end{wrapfigure}
-->
{{< math "definition" "PES" >}} Perfektní eliminační schéma (PES) grafu \(G\) je pořadí vrcholů \(v_1, \ldots, v_n\) t. ž. \(\forall i \in [n]\) platí, že leví sousedé \(v_i\) (\(= \left\{v_j \mid j < i, v_jv_i \in E\right\}\)) tvoří kliku.{{< /math >}}

{{< math "theorem" >}}G je chordální \(\iff\) G má PES.{{< /math >}}

{{< math "proof" >}}\(\Leftarrow\) obměnou nechť \(G\) není chordální a má tedy indukovanou kružnici velikosti alespoň \(4\). Pro spor nechť máme PES. Nejpravější vrchol špatné kružnice má na této kružnici dva levé sousedy, kteří spolu nesousedí, což je spor, což je spor s definicí PES.
...
{{< math "consequence" >}}pro daný graf \(G\) lze v polynomiálním čase rozhodnout, zda je chordální.{{< /math >}}

{{< math "proof" >}}Trháme simpliciální vrcholy, které chordální graf musí vždy mít -- ty umíme v polynomiálním čase najít otestováním všech sousedů. Pokud simpliciální vrchol v nějakém bodě nenajdeme, tak graf chordální být nemohl.{{< /math >}}

{{< math "consequence" >}}chordální grafy jsou perfektní.{{< /math >}}

{{< math "proof" >}}Je-li graf \(G\) chordální, pak má PES, pomocí kterého ho umíme obarvit tak, aby měl nejvýše \(\omega(G)\) barev. Jelikož je navíc každý indukovaný podgraf chordálního grafu také chordální, tak platí i pro indukované podgrafy, což potřebujeme pro perfektnost.{{< /math >}}

{{< math "definition" >}}\(G\) je hamiltonovský, pokud má kružnici na \(n\) vrcholech (jako podgraf).{{< /math >}}

{{< math "theorem" "Bondyho-Chvátalova" >}}Nechť \(G\) je graf na \(n \ge 3\) vrcholech. Nechť \(x,y\) jsou nesousedé t. ž. \(\deg_G(x) + \deg_G(y) \ge n\). Nechť \(G^+ = (V, E \cup \left\{xy\right\})\). Pak \(G\) je hamiltonovský \(\iff\) \(G^+\) je hamiltonovský.{{< /math >}}
...
{{< math "theorem" "Dirac" >}}\(G\) graf na \(n \ge 3\) vrcholech s min. stupněm \(\ge n/2\) je hamiltonovský.{{< /math >}}

{{< math "proof" >}}Z Bondy-Chvátalovy věty doplníme na \(K_n\), který je hamiltonovský.{{< /math >}}

### 9. přednáška

#### Tutteův polynom

{{< math "definition:" "multigraf" >}} \(G = (V, E)\) kde \(V\) jsou vrcholy a \(E\) multimnožina prvků z \(V \cup \binom{V}{2}\){{< /math >}}
- odstranění a kontrakce fungují intuitivně -- kontrakce nezahazuje hrany, protože máme multigraf

{{< math "definition" "most" >}}hrana \(e \in E\) je most, v multigrafu \(G\), pokud \(G - e\) má více komponent než \(G\){{< /math >}}
- \(k(G) = k(V, E) = \) počet komponent

{{< math "definition:" "hodnost/rank" >}}\(E\) je \(r(E) := |V| - k(G)\){{< /math >}}
...
{{< math "definition:" "nulita" >}}\(E\) je \(n(E) := |E| - r(E)\){{< /math >}}
...
{{< math "definition:" "Tutteův polynom" >}}multigrafu \(G = (V, E)\) je polynom proměnných \(x, y\) definovaný jako \[T_G(x, y) := \sum_{F \subseteq E} (x - 1)^{r(E) - r(F)} \cdot (y - 1)^{n(F)}\]{{< /math >}}

{{< math "claim" >}}pro \(G\) souvislý je \(T_G(1, 1)\) počet koster \(G\){{< /math >}}
...
{{< math "fact" >}} \(\mathbb{R}\llbracket x \rrbracket\) tvoří (komutativní) okruh (máme \(+, \cdot, 0, 1\)){{< /math >}}
...
{{< math "definition" "převrácená hodnota" >}} Převrácená hodnota FMŘ \(A(x)\) je taková FMŘ \(B(x)\), že \(A(x) \cdot B(x) = 1\){{< /math >}}
...
{{< math "claim" >}}Nechť \(A(x) = \sum_{n = 0}^{\infty} a_n x^n\) je FMŘ. Potom \(\frac{1}{A(x)}\) existuje, právě když \(a_0 \neq 0\) (a pak je jednoznačně určena).{{< /math >}}

{{< math "proof" >}}Hledejme inverz. Rozepsání \(A(x) \cdot B(x) = 1 + 0x + 0x^2 + \ldots\) nám dává soustavu takovýchto rovnic, které mají jednoznačné řešení:
...
{{< math "definition" "složení" >}}\(A(x) = \sum a_nx^n, B(x) = \sum b_nx^n\) jsou FMŘ. Složení je \(A(B(x)) = a_0B(x)^0 + a_1B(x)^1 + \ldots\) {{< /math >}}. Obecně je problém to zadefinovat, potřeboval bych znát hodnotu součtu, ale jde to, když:
...
Potom **obyčejná vytvořující funkce** pro \(\mathcal{A}\) je FMŘ \[\mathrm{OVF}(\mathcal{A}) = \sum_{n \ge 0} a_n x^n\]{{< /math >}}
...
- {{< math "observation" >}}\(\mathrm{OVF}(\mathcal{A} \cup \mathcal{B}) = \mathrm{OVF}(\mathcal{A}) + \mathrm{OVF}(\mathcal{B})\){{< /math >}}
- {{< math "observation" >}}\(\mathrm{OVF}(\mathcal{A}) \cdot \mathrm{OVF}(\mathcal{B}) = \mathrm{OVF}(\mathcal{A} \times \mathcal{B})\){{< /math >}}
...
Potom platí, že \(a_n = \sum_{j = 0}^{n} \binom{n}{j} \cdot s_j \cdot k_{n - j}\), tedy \(a_n = \) počet grafů na \(n\) vrcholech majících dvě komponenty souvislosti, z nichž jedna je strom a druhá kružnice:
\[
\begin{aligned}
...
- \(D(x) = S^k(x)\), tedy \(d_n\) je počet uspořádaných \(k\)-tic stromů tvořících rozklad vrcholů
...
3. pro dvě konečné množiny \(V, W \subseteq \mathbb{N}\) t. ž. \(|V| = |W|\) platí, že počet \(\alpha \in \mathcal{A}\) t. ž. \(V(\alpha) = V\) je stejný jako počet \(\alpha \in \mathcal{A}\) t. ž. \(V(\alpha) = W\) (co do počtu, záleží jen na velikosti množiny vrcholů)
...
5. pokud \(\forall \alpha \in \mathcal{A}: V(\alpha) \neq \emptyset\), pak \[\mathrm{exp}(A(x)) = e^{A(x)} = 1 + A(x) + \frac{A^2(x)}{2} + \ldots = \sum_{n \ge 0} f_n \frac{x^n}{n!}\] kde \(f_n\) je počet množin \(\left\{\alpha_1, \ldots, \alpha_k\right\} \subseteq \mathcal{A}\), kde \(V(\alpha_1) \cup \ldots \cup V(\alpha_{k}) = \left\{1, \ldots, n\right\}\)
...
{{< math "observation" >}}Pokud \(\gamma \in \Gamma, \gamma^{-1}\) je inverzní prvek k \(\gamma\), potom \(\forall x, y \in A: \gamma \cdot x = y \iff \gamma^{-1} \cdot y = x\){{< /math >}}

{{< math "consequence" >}}\(\forall p \in \Gamma:\) zobrazení \(x \mapsto p \cdot x\) je bijekce \(A \longleftrightarrow A\){{< /math >}}

### 12. přednáška
{{< math "definition:" "množina pevných bodů" >}}\(\gamma \in \Gamma\), značená \(\mathrm{Fix}(\gamma) = \left\{x \in A \mid \gamma x = x\right\}\){{< /math >}}

{{< math "definition:" "stabilizátor" >}} prvku \(x \in A\) je \(\mathrm{Stab}(x) = \left\{\gamma \in \Gamma \mid \gamma x = x\right\}\){{< /math >}}

{{< math "observation" >}}\(\gamma \in \Gamma, x \in A: \gamma \in \mathrm{Stab}(x) \iff x \in \mathrm{Fix}(\gamma) \iff \gamma x = x\){{< /math >}}
...
{{< math "lemma" "o orbitě a stabilizátoru" >}}Nechť \(\Gamma\) je konečná grupa s akcí na množině \(A\). Potom \[\forall x \in A: |\mathrm{Stab(x)}| \cdot |\left[x\right]| = |\Gamma|\] {{< /math >}}

{{< math "proof" >}}Nechť množina \(\mathrm{Map}(x, y)\) je množina akcí \(a\), pro které \(a \cdot x = y\). Pro akce \(\sigma \in \mathrm{Map}(x, y)\) pomocí \(\sigma^{-1} a\) lze definovat bijekci mezi \(\mathrm{Map}(x, y)\) a \(\mathrm{Map}(x, x)\). Poté \[\forall x \in A, |\Gamma| = \sum_{y \in [x]} |\mathrm{Map}(x, y)| = \sum_{y \in [x]} |\mathrm{Stab}(x)| = |[x]| |\mathrm{Stab}(x)|\]
{{< /math >}}
...
- {{< math "observation" >}}\(k\)-partitní je to samé jako \(k\)-obarvitelný (\(\Chi(G)\)){{< /math >}}
- partity mohou být i prázdné (\(k\)-partitní je i \(k'\)-partitní, pro \(k' \ge k\))

![](turan.svg)

{{< math "observation" >}}\(\forall r \in \mathbb{N}, r \ge 2: \mathrm{ex}(n, K_r) \ge t_{r - 1}(n)\), protože \(T_{r - 1}(n)\) neobsahuje \(K_r\) (z každé partity si klika vezme \(\le 1\) vrchol, tedy nejvýše \(r - 1\)) {{< /math >}}

{{< math "lemma" "1" >}}Každý \(k\)-partitní graf na \(n\) vrcholech má nanejvýš \(t_{k}(n)\) hran.{{< /math >}}
...
- jinak pro spor \(|P_k| \ge |P_1| + 2\)
	...
	- nechť \(x \in P_k\), nechť \(\tilde{G}\) je úplný \(k\)-partitní s partitami \(P_1 \cup \left\{x\right\}, P_2, P_3, \ldots, P_{k} \setminus \left\{x\right\}\); potom \(|E(\tilde{G})| > |E(G)|\), což je spor:
		...
		- stupně pro \(P_1\) klesnou o \(1\) (vrcholy přestanou vidět \(x\))
		- stupně pro \(P_k \setminus \left\{x\right\}\) vzrostou o \(1\) (vrcholy začnou vidět \(x\))
		- stupeň pro \(x\) vzroste alespoň o \(1\) (\(x\) přestane vidět \(P_1\) a začne vidět \(P_k\))
{{< /math >}}

{{< math "lemma" "2" >}}Nechť \(G = (V, E)\) je graf neobsahující \(K_r\) jako podgraf. Potom \(\exists H = (V, E_H)\) \((r-1)\)-partitní t.ž. \(\deg_G(x) \le \deg_H(x)\) (a tudíž \(|E(G)| \le |E(H)|\)){{< /math >}}
- \(=\) pro graf neobsahující \(K_r\) jako podgraf jsou \((r-1)\)-partitní nejlepší

{{< math "proof" >}}indukcí podle \(r\)
- \(r = 2 \Rightarrow G\) neobsahuje \(K_2\) a tedy nemá hrany; \(G = H\) splňuje tvrzení (celé tvoří jednu partitu)
...
{{< math "theorem" "Turán, 1941" >}}\(\forall r \ge 2: \mathrm{ex}(n, K_r) = t_{r - 1}(n)\){{< /math >}}
...
{{< math "remark" >}}\(t_k(n) = \frac{k-1}{k} \binom{n}{2} + \mathcal{O}(n) = \frac{k - 1}{2k} n^2 + \mathcal{O}(n)\){{< /math >}}

##### Pro minory

{{< math "definition" >}}pro graf \(H: \mathrm{ex}_\preceq(n, H)\) je maximální počet hran grafu \(G\) na \(n\) vrcholech bez \(H\) jako minoru.{{< /math >}}

{{< math "observation" >}}\(\mathrm{ex}(n, H) \ge \mathrm{ex}_\preceq(n, H)\), protože graf bez \(H\)-minoru nemá ani \(H\)-podgraf{{< /math >}}
- obráceně platit nemusí.

{{< math "observation" >}}\(\mathrm{ex}_\preceq(n, K_3) = n - 1\) (dostávám stromy!){{< /math >}}

{{< math "theorem" >}}\(\forall r \ge 3 \exists c_r > 0: \forall n: \mathrm{ex}_\preceq(n, K_r) < c_r \cdot n\){{< /math >}}
...
{{< math "definition" >}}\(k\)-uniformní hypergraf je dvojice \((V, E)\), kde \(E \subseteq \binom{V}{k}\){{< /math >}}
...
{{< math "theorem" "Erdős-Ko-Rado, 1961" >}}\(\forall k, n \in \mathbb{N}: n \ge 2k \Rightarrow f(k, n) = \binom{n - 1}{k - 1}\){{< /math >}}

{{< math "proof" >}} dokazujeme dva odhady:

- dolní odhad \(f(k, n) \ge \binom{n - 1}{k - 1}\) ze slunečnicové konstrukce
- horní odhad \(f(k, n) \le \binom{n - 1}{k - 1}\): máme \(H = (V, E)\) \(k\)-uniformní hypergraf t.ž. \(E\) je pronikající systém množin
...
{{< math "observation" >}}pokud \(e = \left\{a_1, \ldots, a_k\right\}\) je vůči \(C\) interval, pak \(\exists \le k - 1\) dalších hran \(e'\) t.ž. jsou intervaly vůči \(C\){{< /math >}}
