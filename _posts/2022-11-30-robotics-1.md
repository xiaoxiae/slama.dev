...
{: .no-invert}
![Singularity example.](/assets/robotics-1/singularities.png)

To remedy this, certain methods exist that balance error and velocity to make the robot movement smoother, like **camped least squares**, which minimizes the sum of minimum error norm on _achieved end-effector velocity_ and minimum norm of _joint velocity_.

##### Pseudo-inverse
In the case that the Jacobian doesn't have an inverse (we can't satisfy \(H \dot{q} = \dot{p}\), either because of singularities or redundancy), we want to find a matrix \(H\) that minimizes \[|| H \dot{q} - \dot{p} ||\]

instead. This turns out to be the **pseudo-inverse** \(J^\#\).
- can be obtained using SVD
- satisfies the following conditions:
	- \(J J^\# J = J\)
	- \(J^\# J J^\# = J^\#\)
	- \((J^\# J)^T = J^\# J\)
	- \((J J^\#)^T = J J^\#\)
	- \(J^\# = J^T (J J^T)^{-1}\) (for \(m < n\))
...
#### In Cartesian Space
- notation here is \(p = p(s) \rightarrow s = s(t)\)
	- as opposed to \(q = q(\lambda) \rightarrow \lambda = \lambda(t)\)
- allows more direct visualization of the generated path
- can account for obstacles + lack of "wandering"
- _needs on-line kinematic inversion_
...
{: .no-invert}
![Bang-coast-bang analysis illustration.](/assets/robotics-1/trapezoid.png)
...
{: .no-invert}
![Fly-over analysis illustration.](/assets/robotics-1/fly-over.png)
...
Say we want to slow a given timing low down or speed it up because of constraints, we don't need to recaculate all of the motion profiles: velocity scales **linearly**, acceleration scales **quadratically**.
...
- we consider all EE velocities that can be obtained by chosing joint velocities of **unit norm**:

\[\dot{q}^T \dot{q} \rightarrow v^T \underbrace{J^{\#T}J^\#}_{\text{ellipsoid}}v = 1\]

{: .no-invert}
![Velocity and force manipulability.](/assets/robotics-1/manipulability.png)

##### Force manipulability
We can apply the exact same concept to forces:

\[\tau^T \tau \rightarrow F^T \underbrace{J^{T}J}_{\text{ellipsoid}}F = 1\]

The comparison between the two is interesting -- they are **orthogonal:**

{: .no-invert}
![Cubic spline illustration](/assets/robotics-1/manipulability.png)
