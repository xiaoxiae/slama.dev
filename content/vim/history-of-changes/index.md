...
categoryPart: 2
description: A short post about how history of changes is implemented in Vim.
toc: true
---
...
### Implementations
Most editors keep a **linear** history of changes made to a given file, meaning that you can only move forward and backward in time. While simple to implement, it isn't an ideal solution -- what if you undid some changes, made new ones and only afterwards realized that the previous changes were actually the ones you wanted to keep?
...
One way we could do this is based on the **relative time** a given change was made. To move relative to the current time, use the {{< inline_highlight "vim" >}}earlier{{< /inline_highlight >}} and {{< inline_highlight "vim" >}}later{{< /inline_highlight >}} commands:
```vim
...
The {{< inline_highlight "vim" >}}<`.>{{< /inline_highlight >}} command is nice, as it takes you back to the line that was last changed. You can also just use `<u>` and `<C-r>` (like I do, although I shouldn't), but the former is far more elegant 🙂.
