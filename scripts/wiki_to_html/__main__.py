...
def get_files_with_extension(source, extension):
    ...
    return [
        file for file in listdir(path.join(source, ".")) if file.endswith(extension)
    ]
...
def write_utf8_file(destination_path, destination_file_name, contents):
    ...
    # write the file
    with codecs.open(
        path.join(destination_path, destination_file_name), "w", encoding="utf8"
    ) as file:
        file.write(contents)
...
def copy_utf8_file(source_path, destination_path, destination_file_name):
    ...
    write_utf8_file(
        destination_path, destination_file_name, read_utf8_file(source_path)
    )
...
def get_config_from_jekyll(source, variable):
    ...
    return (
        compile(r"^" + variable + r': *?" *(.*?) *"', flags=MULTILINE)
        .search(contents)
        .group(1)
    )
...
def substitute_liquid_variables(content, file_name, variables):
    ...
    for variable, substitution in all_variables.items():
        content = sub(
            r"\{\{ *" + variable + r" *\}\}", substitution, content, flags=MULTILINE
        )
...
md_substitutions = [
    (r"(^[^-#\s].*?\n)-", r"\g<1>\n-"),  # prevent incorrect Pandoc formatting of lists
    (r"\[(.+?)\]\(\1\)", r"\[\[\g<1>\]\]"),  # vimwiki wiki links to relative links
]

# substitutions for HTML
html_substitutions = [
    (r"<li><p>(.+)<\/p><\/li>", r"<li>\g<1></li>"),  # fix for <p> inside list
    (r"\[\[(.+?)\]\]", r'<a href="\g<1>.html">\g<1></a>'),  # wiki links
]
...
for md_file_name in get_files_with_extension(wiki_folder, ".md"):
    ...
    html_file_content = perform_regex_substitutions(
        html_file_content, html_substitutions
    )

    #  swap for liquid-like variables
    html_file_content = substitute_liquid_variables(
        html_file_content, md_file_name[:-3].lower(), site_variables
    )
...
copy_utf8_file(
    path.join(includes_folder, "main.css"),
    path.join(output_folder, "assets"),
    "main.css",
)
