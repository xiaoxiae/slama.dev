...
pdf: true
excerpt: Expanded translation of my lecture notes from a Randomized and Approximation Algorithms course that I took, and a more detailed explanation of the topics covered in my video about BEST-SAT.
---
...
{% math theorem %}RAND-SAT is a \(2\)-approximation algorithm.{% endmath %}
...
{% math theorem %}LP-SAT is a \(\left(1 - \frac{1}{e}\right)\)-approximation algorithm.{% endmath %}
...
{% math fact "A - A/G mean inequality" %} \[\prod_{i = 1}^{n} a_i^{\frac{1}{n}} \le \frac{1}{n} \sum_{i = 1}^{n} a_i\]{% endmath %}
...
{% math fact "B - Jensen's inequality" %}if a function is concave on the interval \(\left[0, 1\right]\) and \(f(0) = a, f(1) = a + b\), then \[\forall x \in \left[0, 1\right]: f(x) \ge a + bx\]{% endmath %}

{% math proof %} [https://en.wikipedia.org/wiki/Jensen%27s_inequality](https://en.wikipedia.org/wiki/Jensen%27s_inequality)
{% endmath %}


{% math fact "C - 1/e inequality" %}\[\left(1 - \frac{1}{n}\right)^n \le \frac{1}{e}\]{% endmath %}
...
{% math theorem %}BEST-SAT is \(\frac{3}{4}\)-approximation.{% endmath %}
