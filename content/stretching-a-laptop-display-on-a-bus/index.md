...
What the global elites are not telling your is that there is a **third option:** combine the comfort of a non-vampiric hand position and an undistorted dispaly by folding it and **stretching the screen** to accomodate for the distortion -- all for the low low price of a single magical `xrandr` incantation:
...
- should onlt transform to nearest multiples of the angle, so we don't keep jitering as the eye tracking stabilizes
...
- we can also rotate so the screeen is level with the eyes
