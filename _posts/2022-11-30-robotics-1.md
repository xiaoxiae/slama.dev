...
1. **Newton method:**
	- uses Taylor series around point \(q^k\) (in the \(k\)-th iteration) to expand the forward kinematics
	- \(r = F(q) = F(q^k) + J_r(q^k)(q - q^k) + \underbrace{\mathcal{O}(||q-q^k||^2)}_{\text{neglected}} \)
		- here \(J_r(q) = \frac{\partial F}{\partial q}\) (analytical Jacobian)
	- solving for \(q\), we get \[q^{k + 1} = q^k + J^{-1}_r(q^k) \underbrace{(r - F(q^k)}_{\text{error}})\]
	- is nice when \(n = m\), otherwise we have to use the pseudo inverse
	- bad for problems near singularities (becomes unstable)

2. **Gradient method:**
	- aims to minimize the error function \[H(q) = \frac{1}{2} ||r - F(q)|| = \frac{1}{2} [r - F(q)]^T [r - F(q)]\]
	- deriving[^derivation] \(H(q)\) we get \[q^{k + 1} = q^k + \alpha J_r(q)^T [r - F(q)]\]
...
[^derivation]: \[ \begin{aligned} H(q) &= \frac{1}{2} [r - F(q)]^T [r - F(q)] \\ \frac{\partial H}{\partial q} &= \frac{1}{2} \cdot 2 \cdot (-1) \cdot [r - F(q)]^T J_r(q) \\ \nabla_q H(q) &= -J_r(q)^T [r - F(q)] \\ \end{aligned} \]
...
- determined by the **Jacobian matrix** \(J\)
	- can be obtained either geometrically or by time differenciation
...
We get \[p_{0,1} = (a_1 c_1, a_1 s_1, 0)^T \quad p_{0, EE} = (a_1 c_1 + a_2 c_{12}, a_1 s_1 + a_2 s_{12}, 0)^T\]
...
##### Higher-orders
For calculating other differential relations, we can just continue derivating:
...
#### Force Kinematics
- again use the Jacobian to relate **forces** of the end effector to those of the joints
...
{% math ENdefinition "trajectory" %}a path on which a timing is specified (velocitiyes, accelerations, etc.).{% endmath %}
...
- stop in a point, move at constant velocity
...
##### Cubic polynomial
Useful for interpolating between two positions (\(q_0, q_1\)) given the starting and ending velocity (\(v_0, v_1\)).
...
- \(q(0) = q_0 \implies d = 0\)
- \(q'(0) = \Delta q \cdot c \implies c = v_0 / \Delta q\)
- \(q(1) = q_1 \implies a + b + c = 1\)
- \(q'(1) = \Delta q \cdot (3a + 2b + c) \implies 3a + 2b + c = v_1 / \Delta q\)

##### Splines
When we have \(N\) knots and want continuity up to the second derivative, we can **concatenate \(N - 1\) cubic polynomials**
- _minimizes the curvature among all interpolating functions_ (with 2nd derivatives)
- \(4(N - 1)\) coefficients
- \(4(N - 1) - 2\) conditions:
