...
def get_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="the separator between individual files; none by default",
    )
...
for i, name in enumerate(arguments.files):
    ...
    if not os.path.exists(name):
        sys.exit(f"The Xournal file '{name}' not found, exiting.")
    ...
    if arguments.separator is not None and i != len(arguments.files) - 1:
