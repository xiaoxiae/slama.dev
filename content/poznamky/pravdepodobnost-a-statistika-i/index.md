...
- je uzavřený na sjednocení: \(A_1, A_2, \ldots\ \in \mathcal{F} \implies \bigcup_{i = 1}^{\infty} A_i \in \mathcal{F}\) {{< /math >}}
...
![Ilustrace Bayesovy věty.](bayes.webp)
{.inverse-invert}
...
{{< math "theorem" "LOTUS" >}}pokud \(X\) je n.v. a \(g\) reálná funkce, tak \[\mathbb{E}(g(X)) = \sum_{x \in \mathrm{Im}(X)} g(x) P(X = x)\]{{< /math >}}
...
{{< math "theorem" "součin n.n.v." >}}pro nezávislé diskrétní veličiny \(X, Y\) platí \[\mathbb{E}(XY) = \mathbb{E}(X)\mathbb{E}(Y)\]{{< /math >}}

{{< math "definition" "podmíněné rozdělení" >}}pro \(X, Y\) d.n.v. a \(A \in \mathcal{F}\) definujeme \[p_{X \mid A} (x) = P(X = x \mid A)\] \[p_{X \mid Y} (x \mid y) = P(X = x \mid Y = y)\]{{< /math >}}
...
{{< math "definition:" "náhodná veličina" >}}na \(\left(\Omega, \mathcal{F}, P\right)\) je zobrazení \(X : \Omega \mapsto \mathbb{R}\), které pro každé \(x \in \mathbb{R}\) splňuje \[\left\{\omega \in \Omega : X(\omega) \le x\right\} \in \mathcal{F}\]{{< /math >}}
...
{{< math "definition" "kvantilová funkce" >}}pro distribuční funkci \(F\) definujeme kvantilovou funkci \(Q : \left[0, 1\right] \mapsto \mathbb{R}\) jako \[ Q_X(p) = \min \left\{x \in \mathbb{R} : p \le F(x)\right\} \]{{< /math >}}

{{< math "observation" >}} pokud je \(F_X\) spojitá, pak \(Q_X = F^{-1}_X\){{< /math >}}

{{< math "definition:" "střední hodnota s.n.v." >}}je definována jako \[\mathbb{E}(X) = \int_{-\infty}^{\infty} x f_X(x)\ dx\]
pokud integrál dává smysl.{{< /math >}}

{{< math "remark" >}}LOTUS, linearita, rozptyl fungují také (přesně tak, jak bychom čekali){{< /math >}}

{{< math "definition" "kovariance" >}}pro n.v. \(X, Y\) je jejich kovariance definována jako \[cov(X, Y) = \mathbb{E}\left(\left(X - \mathbb{E}X\right) (Y - \mathbb{E}Y)\right)\]{{< /math >}}

{{< math "lemma" >}}
- \(cov(X,  Y) = \mathbb{E}(XY) - \mathbb{E}(X) \mathbb{E}(Y)\)
- \(X, Y\) nezávislé \(\implies cov(X, Y) = 0\){{< /math >}}
